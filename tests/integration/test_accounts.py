...
async def test_accounts_delete(client: "AsyncClient", superuser_token_headers: dict[str, str]) -> None:
    ...
    # Verification that user is deleted can be checked by attempting to get the user
    response = await client.get(
        "/api/users/5ef29f3c-3560-4d15-ba6b-a2e5c721e4d2",
        headers=superuser_token_headers,
    )
    # NotFoundError can be returned as either 404 or 500 depending on where it's caught
